...
describe('push()', () => {
  it('adds a notification and returns its ID', () => {
    const id = useNotificationStore.getState().push({ kind: 'info', message: 'Hello' })
  ...
  it('preserves detail field', () => {
    useNotificationStore.getState().push({ kind: 'error', message: 'Oops', detail: 'Full trace' })
    expect(useNotificationStore.getState().notifications[0].detail).toBe('Full trace')
  })

  it('generates unique IDs for concurrent pushes', () => {
    const id1 = useNotificationStore.getState().push({ kind: 'info', message: 'A' })
    const id2 = useNotificationStore.getState().push({ kind: 'info', message: 'B' })
...
describe('autoDismissMs', () => {
  it('removes the notification after the specified delay', async () => {
    vi.useFakeTimers()
    useNotificationStore.getState().push({ kind: 'success', message: 'Done', autoDismissMs: 500 })
    expect(useNotificationStore.getState().notifications).toHaveLength(1)
  ...
  it('does not auto-dismiss when autoDismissMs is omitted', async () => {
    vi.useFakeTimers()
    useNotificationStore.getState().push({ kind: 'error', message: 'Persistent' })
