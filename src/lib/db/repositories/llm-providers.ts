...
export async function createLlmProvider(data: {
  ...
  const now = Date.now()
  await db.execute(
    `INSERT INTO llm_providers (id, name, provider_type, base_url, api_key, is_default, created_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7)`,
    [id, data.name, data.provider_type, data.base_url, data.api_key ?? null, data.is_default ?? 0, now]
  )
  return id
}
...
export async function listLlmProviders(): Promise<LlmProvider[]> {
  return db.select<LlmProvider>('SELECT * FROM llm_providers ORDER BY created_at ASC')
}

export async function updateLlmProvider(
  id: string,
  data: Partial<Pick<LlmProvider, 'name' | 'provider_type' | 'base_url' | 'api_key'>>
): Promise<void> {
...
export async function setDefaultProvider(id: string): Promise<void> {
  ...
  await db.execute('UPDATE llm_providers SET is_default = 1 WHERE id = $1', [id])
}
