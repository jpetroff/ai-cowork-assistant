...
The system SHALL define a `TableName` TypeScript union type that exactly matches the six tables in the SQLite schema: `projects`, `conversations`, `messages`, `artifacts`, `llm_providers`, `app_settings`.
...
### Requirement: Prisma schema mirrors the SQLite migration
The system SHALL maintain a `prisma/schema.prisma` file whose models exactly match the tables, columns, and types defined in `src-tauri/src/db.rs` migration v1, so that `prisma generate` produces accurate TypeScript entity types.

#### Scenario: All six models present
- **WHEN** `prisma generate` is executed
- **THEN** the generated client exports types `Project`, `Conversation`, `Message`, `Artifact`, `LlmProvider`, `AppSetting`
